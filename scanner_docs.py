...
# NLP opcional (spaCy): idioma e entidades. Sem ele, o scanner segue igual.
try:
    from project.app import nlp as _nlp
except Exception:
    _nlp = None
USE_NLP = _nlp is not None and os.getenv("SCANNER_NLP", "1") == "1"
...
def analyze_text_structure(file_name: str, text: str) -> Dict[str, Any]:
    ...
    lang = _nlp.detect_language(text) if USE_NLP else guess_language(text)
    doc_type = classify_doc_type(text)

    entidades = {"pessoas": [], "organizacoes": [], "locais": []}
    if USE_NLP:
        try:
            entidades = _nlp.extract_entities(text, lang=lang if lang != "desconhecido" else None)
        except Exception as exc:
            print(f"[AVISO] Entidades não extraídas de {file_name}: {exc}")
    ...
    return {
        ...
        "entidades_pessoas": " | ".join(entidades["pessoas"]),
        "entidades_organizacoes": " | ".join(entidades["organizacoes"]),
        "entidades_locais": " | ".join(entidades["locais"]),
    }
...
# Caracteres de controle que XLSX e DOCX (XML) não aceitam.
# Aparecem com frequência em PDFs gerados por LaTeX ou escaneados.
...
def sanitize_df(df: pd.DataFrame) -> pd.DataFrame:
    # Célula a célula: funciona tanto com dtype object (pandas 2)
    # quanto com o dtype de texto padrão do pandas 3.
    if hasattr(df, "map"):
        return df.map(_clean_value)
...
def export_formats(df: pd.DataFrame, out_dir: Path, base_name: str):
    ...
    for fmt in FORMATS:
        ...
        try:
            _export_one(df, fmt, out_file, has_pattern_schema)
            print(f"[OK] Exportado: {out_file}")
        except Exception as exc:
            # Um formato com problema não pode derrubar a varredura inteira
            print(f"[AVISO] Falha ao exportar {out_file}: {exc}")


def _export_one(df: pd.DataFrame, fmt: str, out_file: Path, has_pattern_schema: bool):
    if fmt == "csv":
...
def export_raw_texts(raw_texts: Dict[str, str], saida_base: Path):
    ...
    for fname, text in raw_texts.items():
        ...
        expected.add(out_file.name)
        with open(out_file, "w", encoding="utf-8") as f:
            f.write(text)
        print(f"[OK] Texto bruto salvo: {out_file}")

    # Remove textos de documentos que não existem mais, para o RAG
    # não responder com base em arquivos apagados.
    for old in base.glob("*.txt"):
        if old.name not in expected:
            old.unlink()
            print(f"[OK] Texto obsoleto removido: {old}")

# Sempre regravados, mesmo vazios: se a pasta ficar sem documentos,
# o índice antigo não pode continuar aparecendo no app.
def export_docs_index(docs_index: List[Dict[str, Any]], saida_base: Path):
    export_formats(pd.DataFrame(docs_index), saida_base, "_docs_index")

def export_docs_analysis(docs_analysis: List[Dict[str, Any]], saida_base: Path):
    export_formats(pd.DataFrame(docs_analysis), saida_base, "analise_detalhada")
...
def export_all(df_matches: pd.DataFrame,
    ...
    # Resultados por padrão/arquivo são regenerados do zero a cada varredura
    for sub in ("geral", "por_padrao", "por_arquivo"):
        shutil.rmtree(saida_base / sub, ignore_errors=True)
