...
RAG_TIMEOUT = 1.5
PRELOAD_TIMEOUT = float(os.getenv("OLLAMA_PRELOAD_TIMEOUT", "300"))
OLLAMA_TIMEOUT = 60
...
def preload_model(model: str = MODEL_MAIN):
    """Pede ao Ollama para carregar o modelo na memória (sem gerar texto)."""
    start = time.time()
    try:
        resp = requests.post(
            OLLAMA_URL,
        ...
        if resp.status_code == 404:
            print(f"[PRELOAD] Modelo {model} não encontrado. Rode: ollama pull {model}")
            return
        resp.raise_for_status()
        print(f"[PRELOAD] {model} carregado em {time.time() - start:.1f} s")
